...
class Trainer:
    ...
    def keep_training(
        ...
        if (now - self._last_runstats_time > runstats_interval_minutes * 60 or
            ...
            with self.summary_writer.as_default():
                ...
                if time_per_iteration:
                    tf.summary.scalar('runstats/iterPerSec',
                                      float(1 / time_per_iteration))

                rusage = resource.getrusage(resource.RUSAGE_SELF)
                tf.summary.scalar('runstats/maxrssMB',
                                  rusage.ru_maxrss // 2**10)
                tf.summary.scalar('runstats/swaps', rusage.ru_nswap)
                tf.summary.scalar('runstats/fileInputs', rusage.ru_inblock)
                tf.summary.scalar('runstats/fileOutputs', rusage.ru_oublock)
                tf.summary.scalar('runstats/pageFaults_minor', rusage.ru_minflt)
                tf.summary.scalar('runstats/pageFaults_major', rusage.ru_majflt)
                tf.summary.scalar('runstats/contextSwitches_voluntary',
                                  rusage.ru_nvcsw)
                tf.summary.scalar('runstats/contextSwitches_involuntary',
                                  rusage.ru_nivcsw)
                ...
                if cpu_times:
                    ...
                    tf.summary.scalar('runstats/cpuLoad', avg_cpu_load)
                    tf.summary.scalar('runstats/cpuLoadUser', cpu_times[0])
                    tf.summary.scalar('runstats/cpuLoadSys', cpu_times[1])
                ...
                if gpu_stats:
                    for gpu_idx, stat in gpu_stats.items():
                        ...
                        for k in keys:
                            tf.summary.scalar(
                                'runstats/gpu{0}/{1}'.format(gpu_idx, k),
                                stat[k])
        ...
        if step_var_increment:
            step_var.assign_add(step_var_increment)
