...
class FluidErrors:
    ...
    def add_errors(self,
        ...
        self.fid.append(int(current_frame))
        self.err.append(np.mean(_distance(predicted_pos, gt_pos)))
    ...
    def save(self, path):
        ...
        #print(self.fid)
        #print(self.err)
        #res=(self.fid,self.err)
        filename=path+"fiderr.txt"
    #    np.savetxt(path+"fiderr.txt",res, newline=" ")
        ndata=len(self.fid)
        with open(filename,'w') as f:
            for i in range(ndata):
                f.write("{:d} {:f}\n".format(self.fid[i],self.err[i]))
